...
public class GUIManager implements UserMenu, BudgetViewer, BudgetCalculator {
	...
	public void loadMenuFile(String filePath) {
		try {
			...
			menuWindow.setAviso("O arquivo selecionado nao � valido como arquivo de hist�rico");
		}
	}
		...
		if (validHistoricFile) {
			if (actualYear != lastYearPredicted) {
				...
				calculatorWindow.showTextUser(
						"Selecione a data limite para fazer modifica��es no ano que vem inserindo o m�s e dia no formato mm/dd");
			} else {
				menuWindow.setAviso("Voc� j� fez previs�o nesse ano para o ano que vem");
			}

		} else {
			menuWindow.setAviso(
					"Nenhum arquivo hist�rico foi selecionado. Por favor selecione um arquivo v�lido para poder prosseguir");
		}
	}
	...
	public void viewCalculatedPrevisions() {
		if (lastYearPredicted != 0) {
			...
			menuWindow.setAviso("N�o h� previs�es a serem vistas ainda");
		}
	}
	...
	public void modifyActualPrevision() {
		...
		try {
			...
			if (today.before(lastLimitDate)) {
				...
				menuWindow.setAviso("N�o h� previs�o atual disponivel para modificar");
				modifyng = false;
			}
	...
	private void openModifyPrevisionWindow() {
		...
		calculatorWindow.openWindow();
		calculatorWindow.showTextUser("Selecione um m�s");
		modifyng = true;
	}
	...
	public void selectedMonthView(String month, int year) {
		...
		viewerWindow.showTextUser("V� escolhendo entre os meses que quiser ou troque de ano");
	...
	public void selectedYearView(String month, Integer year) {
		int yearIndex = year - 1;
		budgetItems = uiController.getBudgetItemInformation(yearIndex, true);
		viewerWindow.showTextUser("Escolha um m�s");
	...
	public void loadViewerWindowFile(String filePath) {
		try {
			...
			System.out.println("O arquivo selecionado n�o foi encontrado.");
		} catch (IOException e) {
			System.out.println("O arquivo selecionado n�o � v�lido como arquivo de realizado mensal");
		}
	}
	...
	private void loadItemsCalculatorWindow(int monthIndex) {
		if (completedMonths[monthIndex] == false) {
			...
			calculatorWindow.showTextUser("N�o h� nada que mostrar esse m�s, todas as rubricas de "
